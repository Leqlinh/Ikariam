...
//    public Munition mun = new Munition(this);
...
//        enemyReserve = new RealArmy();
    ...
    public void setArtilleryClass() {
        int index = countByType(artillery, Army.Unit.Mortar);//duyệt trong hàng artillery có bn slot chứa mortar
...
//                            i++;
 ...
 //       mun.setMunition();
   ...
   /* public void setMunition() {
        Munition mun = new Munition(this);
        for (Army.ClassUnit classUnit : Army.ClassUnit.values()) {
            for (Slot slot : this.getMyClassUnit(classUnit)) {
                    if (slot.getUnitType() == Army.Unit.Sulfur){
                        slot.setTotalMunition(mun.getMunitionOfSulfur());
                    }
                    if (slot.getUnitType() == Army.Unit.Archer){
                        slot.setTotalMunition(mun.getMunitionOfArcher());
                    }
                    if (slot.getUnitType() == Army.Unit.Slinger){
                        slot.setTotalMunition(mun.getMunitionOfSlinger());
                    }
                    if (slot.getUnitType() == Army.Unit.Gyrocopter){
                        slot.setTotalMunition(mun.getMunitionOfGyrocopter());
                    }
                    if (slot.getUnitType() == Army.Unit.Balloon){
                        slot.setTotalMunition(mun.getMunitionOfBB());
                    }
                    if (slot.getUnitType() == Army.Unit.Mortar){
                        slot.setTotalMunition(mun.getMunitionOfMortar());
                    }
                    if (slot.getUnitType() == Army.Unit.Catapult){
                        slot.setTotalMunition(mun.getMunitionOfCatapult());
                    }
            }
        }
    }
