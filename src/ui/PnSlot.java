...
public class PnSlot extends JPanel {
    ...
    public PnSlot(BattleField battleField, Army.ClassUnit classUnit, int index) {
        ...
        this.munition = new Munition(battleField);
        addControls();
        addEvents();
    }
    ...
    public void updates(int previewOrReal) {
        if(!"".equals(slot.getImageUrl())){
            ...
            System.out.println("1 " + this.battleField.reserve.getSC().size());
            munition.setMunition();
            System.out.println("2 " + slot.getTotalMunition());
            System.out.println("3 " + slot.getUnitAlive());
            if (previewOrReal == 1) {
